        ...
        cv.check(target, {
          ...
          if (program.verbose >= 1) {
            switch (result) {
              case 0: {
                console.log('File ' + path.basename(target) +
                  ' matches the expected version ' + version)
                break
              }
              case 1: {
                console.log('File ' + path.basename(target) +
              ...
              case 2: {
                console.log('File ' + path.basename(target) +
                  ` doesn't match the expected version ${version}`)
                break
              }
              default:
                throw new Error('Unexpected result by checkVersion')
            }
            return result
          }
        })
